...
class Model:
    def __init__(self):
        ...
        self.model_win = ProbPredictor()
        self.model_lose = ProbPredictor()
        self.model_draw = ProbPredictor()
        self.train_data: List[List[float]] = []
    ...
    def get_bet_size(self, min_bet, max_bet, br_norm):
        return max(min_bet, min(max_bet, self.bankroll[-1] / br_norm))
    ...
    def train_models(self):
        ...
        self.model_win = train_predictor(model=ProbPredictor(), data=data_win[-DATA_SIZE:], plot=True)
        self.model_lose = train_predictor(model=ProbPredictor(), data=data_lose[-DATA_SIZE:], plot=True)
        self.model_draw = train_predictor(model=ProbPredictor(), data=data_draw[-DATA_SIZE:], plot=True)
    ...
    def place_bets(self, opps: pd.DataFrame, summary: pd.DataFrame, inc: pd.DataFrame):
        ...
        if (self.first or RETRAIN_MODELS) and self.days % 360 == 0:
        ...
        if self.bankroll[-1] < 20000:
            global DIFF_TRESHOLD
            DIFF_TRESHOLD = 0.3
            print("changing treshold", file=stderr)
        ...
        self.bet_size.append(self.get_bet_size(min_bet, max_bet, 100))
